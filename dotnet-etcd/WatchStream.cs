...
namespace dotnet_etcd
{
    ...
    {
        private readonly IAsyncDuplexStreamingCall<Etcdserverpb.WatchRequest, Etcdserverpb.WatchResponse> _streamingCall;
        private readonly ConcurrentDictionary<long, Action<Etcdserverpb.WatchResponse>> _callbacks = new ConcurrentDictionary<long, Action<Etcdserverpb.WatchResponse>>();
        ...
        public WatchStream(IAsyncDuplexStreamingCall<Etcdserverpb.WatchRequest, Etcdserverpb.WatchResponse> streamingCall)
        {
        ...
        public async Task CreateWatchAsync(Etcdserverpb.WatchRequest request, Action<Etcdserverpb.WatchResponse> callback)
        {
        ...
        public async Task CancelWatchAsync(long watchId)
        {
            ...
            {
                CancelRequest = new Etcdserverpb.WatchCancelRequest
                {
                    WatchId = watchId
                }
            };
            ...
            try
            {
                while (await _streamingCall.ResponseStream.MoveNext(_cts.Token))
                {
                    ...
                    if (response.Created)
                    {
                        ...
                        if (oldestPendingKey.HasValue && _callbacks.TryRemove(oldestPendingKey.Value, out var callback))
                        {
                    ...
                    else if (_callbacks.TryGetValue(response.WatchId, out var cb))
                    {
                        cb(response);
                    }

...
namespace dotnet_etcd
{
    ...
    public partial class EtcdClient
    {
        ...
        public Task<long> WatchAsync(WatchRequest request, Action<WatchResponse> method, Metadata headers = null,
            ...
            CancellationToken cancellationToken = default) => _watchManager.WatchAsync(request, method, headers, deadline, cancellationToken);
        ...
        public async Task<long[]> WatchAsync(WatchRequest[] requests, Action<WatchResponse> method, Metadata headers = null,
            ...
            CancellationToken cancellationToken = default)
        {
            var watchIds = new long[requests.Length];

            for (int i = 0; i < requests.Length; i++)
            {
                watchIds[i] = await _watchManager.WatchAsync(requests[i], method, headers, deadline, cancellationToken).ConfigureAwait(false);
            }
        ...
        /// <param name="cancellationToken">An optional token for canceling the call.</param>
        /// <returns>A task that completes with an array of watch IDs</returns>
        public async Task<long[]> WatchAsync(WatchRequest[] requests, Action<WatchResponse>[] methods, Metadata headers = null,
            DateTime? deadline = null,
            CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < requests.Length; i++)
            {
                watchIds[i] = await _watchManager.WatchAsync(requests[i], methods[i], headers, deadline, cancellationToken).ConfigureAwait(false);
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            Action<WatchResponse> wrapper = response =>
            {
                var events = response.Events.Select(i =>
                {
        ...
        public long[] WatchRange(string[] paths, Action<WatchResponse> method, Metadata headers = null,
            ...
            CancellationToken cancellationToken = default)
        {
            var watchIds = new long[paths.Length];
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                watchIds[i] = await WatchRangeAsync(paths[i], method, headers, deadline, cancellationToken).ConfigureAwait(false);
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                watchIds[i] = await WatchRangeAsync(paths[i], methods[i], headers, deadline, cancellationToken).ConfigureAwait(false);
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            for (int i = 0; i < requests.Length; i++)
            {
                ...
                Action<WatchResponse> wrapper = response =>
                {
                    var events = response.Events.Select(e =>
                    {
                        return new WatchEvent
                        {
        ...
        /// <param name="cancellationToken">An optional token for canceling the call.</param>
        /// <returns>A task that completes with an array of watch IDs</returns>
        public async Task<long[]> WatchAsync(WatchRequest[] requests, Action<WatchEvent[]>[] methods, Metadata headers = null,
            DateTime? deadline = null,
            CancellationToken cancellationToken = default)
        {
            if (requests.Length != methods.Length)
            ...
            for (int i = 0; i < requests.Length; i++)
            {
                ...
                watchIds[i] = await _watchManager.WatchAsync(requests[i], wrapper, headers, deadline, cancellationToken).ConfigureAwait(false);
            }
